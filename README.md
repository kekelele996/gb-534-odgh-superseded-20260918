...
- 复核与审计：强制发起人、复核人、确认人三人分离（标记已复核者不能确认、发起人不能确认）；退回调查清空原复核结论并记录退回原因，重新复核后才恢复确认资格；确认在单个条件事务内一次性写入确认人、确认时间与审计记录，重复或并发确认仅一次成功，任一步失败全部回滚。记录 request ID、前后快照、输入哈希、算法版本和耗时。
...
| `analyst` | `analyst123` | 数据分析师 | 时序导入/处理与分析发起 |
| `reviewer` | `reviewer123` | 独立复核人 | 分析确认（第三人）与审计读取 |
...
分析状态流（`reviewed -> investigating` 即“退回调查”，会清空原复核结论与复核人，仅保留退回原因；重新复核写入新复核人后才可确认）：

```text
queued -> analyzing -> completed -> reviewed -> confirmed
            |              |          +-----> investigating -> reviewed -> confirmed
            +-> failed     +-----------------> voided
```

职责分离：分析由数据分析师发起、工艺科学家复核、独立复核人确认，三人必须互不相同。确认把确认人、确认时间与审计记录放在同一个带状态前置条件的事务中提交，重复点击或并发请求只有一次成功，任何一步失败状态、复核意见与确认信息都保持原样。
...
- 分析返回 403：按演示角色权限执行操作（分析师发起、科学家复核、独立复核人确认）；无权限的角色在路由层即被拒绝。
- 分析返回 409 `DUTY_SEPARATION_VIOLATION`：发起人不能复核或确认自己的结果，标记已复核的人也不能确认；必须由未参与前两步的第三人确认。
- 分析返回 422 `RETURN_REASON_REQUIRED`：退回调查必须填写非空退回原因；退回后原复核结论被清空，需重新复核才会恢复确认资格。
